...
/**
 * @see <a href="https://codility.com/demo/take-sample-test/tape_equilibrium">Tape Equilibrium lesson</a>.
 */
public class Solution {
	...
	/**
	 * Same as {@link #solution(int[]) solution}, but without extra memory usage.
	 */
	public int unintelligibleSolution (int[] A) {
		...
		for (int i = 1; i <= A.length-1; i++) {
			minimalDifference = Math.min(
					minimalDifference, 
					Math.abs(A[i]+A[i]-A[0]));
		}
